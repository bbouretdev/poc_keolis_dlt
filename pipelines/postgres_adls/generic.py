...
def run_export_pipeline():
    ...
    print(f"📦 Source : {source_schema}.{source_table} -> Cible : {target_name}.parquet")
    ...
    # Définition du chemin cible relatif depuis la racine du conteneur
    if target_name != source_table:
        print(f"✏️ Définition du chemin cible DLT : '{source_table}' -> '{target_name}'")
        resource = resource.with_name(target_name)

    # -------------------------------------------------------------------------
    # 3. EXÉCUTION DU PIPELINE DLT (SINK FILESYSTEM RACINE)
    # -------------------------------------------------------------------------
